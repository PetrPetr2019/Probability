...
namespace Numerical_method
{
    public class IntegralMethods
    {
        public static double CalculatePkDF(int k, double Tbeg, double Tend, double eps, int N)
        {
            ...
            if (k == 1)
                current_integral += (1.0 - Functions.FunctionF(Tend - Tbeg)) * Functions.FunctionDF(Tbeg) + (1.0 - Functions.FunctionF(Tend - Tend)) * Functions.FunctionDF(Tend);
            else
                current_integral += CalculatePkDF(k - 1, Tbeg, Tend, eps, N) + CalculatePkDF(k - 1, Tbeg, Tend, eps, N);
        ...
        public static double CalculateFvkDFv(int k, double Tbeg, double Tend, double eps, int N)
        {
            ...
            if (k == 1)
                current_integral += Functions.FunctionFv(Tbeg) * Functions.FunctionDFv(Tbeg) + Functions.FunctionFv(Tend - Tend) * Functions.FunctionDFv(Tend);
            else
                current_integral += CalculateFvkDFv(k - 1, Tbeg, Tend, eps, N) + CalculateFvkDFv(k - 1, Tbeg, Tend, eps, N);
